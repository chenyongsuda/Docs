...
//     Random victimRNG;
     ...
     protected FJTaskRunner(FJTaskRunnerGroup g,String name) {
    	 setName(name);
         group = g;
         victimRNG = new Random();
     ...
     protected final static class VolatileTaskRef {
         ...
         protected final void put(FJTask r) {
        	 Fib t = (Fib)r;
        	 System.out.println(Thread.currentThread().getName()+"put Task ["+t.s+","+t.e+"]");
             ref = r;
         }
         ...
         protected final FJTask get() {
        	 Fib t = (Fib)ref;
        	 System.out.println(Thread.currentThread().getName()+"Take Task ["+t.s+","+t.e+"]");
             return ref;
         }
 ...
 for (int j = oldcap; j < newcap; ++j) newdeq[j] = new VolatileTaskRef();
	...
	protected void scan(final FJTask waitingFor) {
		 ...
		 * 
		 * This differs slightly from cilk version that uses a random index for
		 * each attempted steal. Exhaustive scanning might impede analytic
		 * tractablity of the scheduling policy, but makes it much easier to
		 * deal with startup and shutdown.
		 */
