 ...
 * return f.getAnswer();
 ...
 public class FJTaskRunnerGroup implements Executor {
     ...
     protected final LinkedBlockingQueue entryQueue = new LinkedBlockingQueue();
 ...
 Thread   current = Thread.currentThread();
     ...
     protected FJTask pollEntryQueue() {
         try {
             FJTask t = (FJTask) (entryQueue.poll());
             return t;
         } catch (Exception  ex) { // ignore interrupts
 Thread.currentThread().interrupt();
             return null;
         }
     }
 ...
 Thread.currentThread().interrupt();
     ...
     protected void initializeThreads() {
         for (int i = 0; i < threads.length; ++i) 
        	 threads[i] = new FJTaskRunner(this,"Working-"+i);
     }
